...
```{r}
data <- read_delim(file = here("data", "clim.txt"), delim = " ")

data_annual <- data %>%
  group_by(year, wy) %>% 
  summarize(total_precip = sum(precip), min_temp = min(tmin_c))

data_feb <- data_annual %>% filter(year == 2)
data_jan <- data_annual %>% filter(year == 1)
```
...
```{r}
yield_anom <- data.frame("year" = unique(data_annual$wy)) %>% mutate(anomaly = NA)
